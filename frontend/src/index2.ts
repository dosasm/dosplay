...
emulators.pathPrefix="/dosplay/jsdos/";
...
async function runBundle(bundle: Uint8Array, options: { x: boolean, worker: boolean }) {
    ...
    const ci:CommandInterface = await (options.worker ?
...
function downloadBundleAndRun(options: { x: boolean, worker: boolean }) {
    ...
    const bundleUrl = `/dosplay/jsdos/bundle/${ele.value}.jsdos?timestamp=` + Date.now();
